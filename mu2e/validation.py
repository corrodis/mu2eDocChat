...
class BenchmarkGenerator:
    ...
    def reform_questions(self, num=None):
        self.dataset = []
        base = Path.home()
        print(base)
        path = "/home/newg2/MLAI/mu2eDocChatV2/mu2e/mcp/docdb/resources/benchmark_questions.json"
        print(path)
        ...
        for entry in data:
            if count <= num:
                ...
                prompt = f"Using this mu2e document specific question: {question}\n\n and keypoint: {keypoint}\n\n Reformulate the question to instead be a query including keywords that an LLM would use to make a call to an mcp server. Format: Respond with only the new query. "
                ...
                self.dataset.append({"doc_id":idx, 
...
"""
    ...
    def check_queries(self, collection, filename='keyword_queries', num_results=7000):
